...
func _ready() -> void:
	...
	for i in Chapitres.CHAPITRES_PAR_MONDE:
		...
		for cote in ["left", "right"]: marge.add_theme_constant_override("margin_" + cote, 18)
...
func _rafraichir() -> void:
	...
	for i in Chapitres.CHAPITRES_PAR_MONDE:
		...
		_zones_chapitres[i].self_modulate = Color.WHITE if accessible else Color("a6afc5")
		_icones_chapitres[i].modulate = Color.WHITE if accessible else Color("a6afc5")
