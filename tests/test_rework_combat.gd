...
func test_sept_coups_et_soins_bornes(v: Verif) -> void:
	...
	v.presque(stats.pv, 55.0, "le boss rend trente PV meme apres epuisement du soin")
...
func test_les_armes_echangent_cadence_et_impact(v: Verif) -> void:
	...
	for arme in [lourd, rapide]:
		...
		v.vrai(ratio > 0.8 and ratio < 1.1, "la puissance brute reste comparable avec la perforation du sceptre")
