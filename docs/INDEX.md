...
| Héros actif : modèle et matières | `tools/blender/apprenti_accueil_v2.py`, `scripts/presentation/materiaux_apprenti.gd` | `shaders/apprenti_accueil_surface.gdshader`, `docs/ops/APPRENTI_ACCUEIL_V2.md` |
| Interface | `ui/`, `scripts/presentation/style_azur.gd` | `scripts/presentation/`, `human/interface/` si design demandé |
| Équilibrage | fichier précis dans `data/` | contrôleur consommateur indiqué dans `scripts/INDEX.md` |
...
| Mobile / safe area | `autoload/ecran.gd`, `scripts/joystick_logique.gd` | `docs/ops/MOBILE.md` |
| Export / mise à jour Android demandé | `tools/android_mises_a_jour.py` | `docs/ops/MISES_A_JOUR_ANDROID.md` |
| Bug de compilation | recherche du symbole/chemin | vérification ciblée seulement si demandée, selon `AGENTS.md` |
...
Les notes de `docs/ops/` sont datées : elles expliquent des travaux particuliers,
mais ne remplacent ni `docs/CURRENT.md` pour l'état actif, ni `AGENTS.md` pour le
processus de travail. Leurs anciennes commandes et mesures ne sont pas à rejouer
automatiquement.
