...
static func offre(mode: String, chapitre: int, salles: int, boss: int, victoire: bool,
		niveau_epreuve: int, rangs: Dictionary, objets: Array[String], echecs: int, echecs_sorts := 0) -> Dictionary:
	...
	if mode == "grimoire":
		...
		if victoire:
			...
			if not resultat["sorts"].is_empty(): resultat["chance_sort"] = Recompenses.chance_garantie(Reglages.EPREUVE_GARANTIE_CAPACITE,echecs_sorts)
	elif mode == "mine":
