...
func _dimensionner() -> void:
	...
	var cote_motif := cote * 0.62
	_motif.size = Vector2.ONE * cote_motif
	_motif.position = Vector2((size.x - cote_motif) * 0.5, cote * 0.04)
	_numero.position = Vector2(0.0, cote * 0.62)
	_numero.size = Vector2(size.x, cote * 0.30)
	_numero.add_theme_font_size_override("font_size", maxi(19, roundi(cote * 0.29)))
